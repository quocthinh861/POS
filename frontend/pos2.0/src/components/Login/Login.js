...
import {Container, Wrapper, Title, Form, Input, Button} from './Signup'
const Login = () => {
  ...
  const signin = () => {
    auth.signInWithPopup(provider).catch(alert).then(result => {
      dispatch(setLogIn({name: result.user.displayName, email: result.user.email}))
    });
  }
  ...
  return (
        ...
        <div className={`${!isShow ? " hide" : ""}`}>
            ...
            <div className="login_buttons">
              <button className="login_google" onClick={signin}>
                <svg className="auth-section-icon" width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M19.1968 13.1682C21.041 13.1682 22.2851 13.9489 22.9944 14.6013L25.7662 11.949C24.0639 10.3983 21.8486 9.44653 19.1968 9.44653C15.3555 9.44653 12.038 11.6068 10.423 14.7509L13.5986 17.1679C14.3953 14.8472 16.5996 13.1682 19.1968 13.1682" fill="#EB4335"></path>
              ...
              <button className="login_email" onClick={() => setIsShow(false)}>Sign In with an email</button>
            </div>
          </div> :
          <Container>
              <Wrapper>
